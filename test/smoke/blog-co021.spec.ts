...
test.describe('CO-021: Cash Back vs Points vs Miles blog article', () => {
  const SLUG = 'cash-back-vs-points-vs-miles'
  ...
  // ---------------------------------------------------------------------------
  test('/blog listing shows the cash-back-vs-points-vs-miles post', async ({ page }) => {
  ...
  test('article h1 title matches expected content', async ({ page }) => {
    await gotoArticle(page)
    await expect(page.locator('[data-article-title]')).toContainText(/cash back vs points vs miles/i)
  })

  test('article has unique title tag', async ({ page }) => {
    await gotoArticle(page)
    await expect(page).toHaveTitle(/cash back vs points vs miles/i)
  })
  ...
  test('article has H3 headings', async ({ page }) => {
    await gotoArticle(page)
  ...
  test('article has data tables comparing reward types', async ({ page }) => {
    await gotoArticle(page)
    const tables = page.locator('[data-article-body] table')
    const count = await tables.count()
    expect(count).toBeGreaterThanOrEqual(2)
  })

  test('article scenario table is visible', async ({ page }) => {
    await gotoArticle(page)
    await expect(page.locator('[data-scenario-table]')).toBeVisible()
  })
  ...
  test('article has Article JSON-LD structured data', async ({ page }) => {
    ...
    expect(jsonld.headline).toMatch(/cash back vs points vs miles/i)
  ...
  test('sitemap includes /blog/cash-back-vs-points-vs-miles', async ({ page }) => {
